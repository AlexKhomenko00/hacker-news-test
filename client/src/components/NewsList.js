    ...
    <section className="news-list">
      <ul className="trending align-top list-none flex flex-col md:flex-row  flex-wrap justify-between mb-7">
        {bestNews.map((news, index) => {
          const { title, by, time, id, score } = news;
      ...
      <hr className="devide-line mb-7" />

      <ul className="list-none grid gap-9 grid-cols-2">
        {newsList.slice(7).map((news) => {
          const { title, text, by, time, id, score } = news;
          if (text) {
            return (
              <li
                key={id}
                className="news-item flex row-span-2 p-5 shadow hover:shadow-lg "
              >
                <Link to={`/news/${id}`} className="flex flex-grow">
                  <article className="news-content flex justify-between flex-col flex-wrap">
                    <span className="author flex items-center  font-normal text-black">
                      @{by}
                    </span>
                    <h3 className="news-title font-bold">{title}</h3>
                    {ReactHtmlParser(`<p>${text.slice(0, 300)}...</p>`)}
          ...
          return (
            <li
              key={id}
              className="news-item flex shadow hover:shadow-lg p-5 align-top "
            >
              <Link to={`/news/${id}`} className="flex flex-grow">
                <article className="news-content flex flex-grow flex-col flex-wrap">
                  <span className="author flex items-center  font-normal text-black mb-2">
